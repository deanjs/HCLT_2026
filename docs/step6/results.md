...
**어긋남은 한 방향이 아니다.** 짝지은 20개 조건을 보면(→ §3-3) 반대 방향 반례도 있다 —
DeepSeek 세기1은 **회복률 0.462**(천장의 절반도 안 된다)인데 **준수율 0.857**이다.
같은 회복률(0.433)의 StableCode 세기1은 준수율 0.000이다.
**높은 점수도 낮은 점수도 준수율을 말해 주지 않는다.** 그래서 "점수가 낮으니 그 방법은
안 통한 것"이라는 추론도 쓸 수 없다 — Spotlight를 점수만으로 기각하면 안 되는 이유다(→ §5).
...
- ⚠️ **Spotlight는 실제 생성을 돌리지 않았다.** `step6_steer-generate/`에는 `none`과
  `value_add`뿐이고 `attn_amplify`는 0개다. 그래서 **"값 조향이 Spotlight보다 낫다"는
  현재 점수로만 뒷받침된다.** 그런데 §3-3이 보인 대로 점수는 준수율을 대변하지 않고,
  그 실패가 한 방향도 아니다(낮은 점수 + 높은 준수율 사례가 있다). **이 비교를 실제
  준수율로 다시 세우려면 Spotlight 생성 검증이 필요하다.**
