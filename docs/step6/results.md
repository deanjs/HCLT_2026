# step6 결과 — 처방: 값 조향 vs Spotlight

> **스텝:** step6(처방) · **결과:** `results/step6_steer/` 2184개 · `step6_steer-generate/` **756개** ·
> `step6_steer-crosslayer/` 336개
> **재현:** `python scripts/step6_summary.py --figs docs/step6/figures`

---

## 1. 결과 JSON 읽는 법

> 이 스텝의 결과는 **폴더가 셋**이다.
> `results/step6_steer/`(점수 회복, 2184개) · `step6_steer-generate/`(실제 생성 검증, **756개** — 무개입 84 · 값 조향 336 · Spotlight 336) ·
> `step6_steer-crosslayer/`(층 교차 주입, 336개).
> 공통 뼈대(`step`·`rq`·`condition`·`meta`)는 [`../코드_하네스공통.md`](../코드_하네스공통.md) §5~6,
> 이 값을 만드는 코드는 [`code.md`](code.md) §7.

### 1-1. 먼저 — 각 필드가 무엇을 말하는가

**S와 회복률** — 자는 step3·step5와 같다.

프롬프트 끝에 `"def "`를 붙여 **이름 자리**를 만들고, 그 자리에 올 후보 두 개
(`removeDuplicates` ↔ `remove_duplicates`)의 로그확률을 재서 뺀다.

> **S = logP(준수 표기 후보) − logP(위반 표기 후보)**  (단위: nat, 양수 = 준수 선호)

| 필드 | 어떤 상태의 점수인가 |
|---|---|
| `S_base` | **절벽 바닥** — 지침은 camel인데 앞 코드 12개가 전부 snake. 개입 없음 |
| `S_clean` | 앞 코드가 **전부 준수**인 상태. 문맥 오염이 없을 때의 **천장** |
| `S_int` | 절벽 바닥에 **처방을 건 채로** 잰 점수 |

metrics 최상위의 `compliance_preference`에는 `S_int`가 그대로 한 번 더 들어간다(같은 값).

> **`recovery` = (S_int − S_base) / (S_clean − S_base)** — 0 = 바닥 그대로, 1 = 천장 복귀,
> **>1 = 천장을 지나쳤다.** 이 스텝에서 1을 넘는 값이 흔한데, **되살아난 것이 아니라
> 지나친 것**이다(→ §4-3). `gap`·`undecidable`·`undecidable_gap_min`의 뜻은 step3·5와 같다
> (분모 크기 \|S_clean − S_base\|, 1.0 미만이면 판정 불가). step6은 **판정 불가 0개**.

**처방 두 가지가 각각 무엇인가**

| 값 | 무엇을 하는 처방인가 |
|---|---|
| `method: "value_add"` | **값 조향(우리 것).** 지정한 층의 잔차에 `strength × (camel−snake 방향)`을 더한다 |
| `method: "attn_amplify"` | **Spotlight(기존 접근 재구현).** 전 층·전 헤드에서 지침 스팬의 어텐션 비중을 `psi_target`까지 끌어올린다 |
| `method: "none"` | 무개입 — 하한선 |

> `method`는 **조건에 적힌 이름**이고, `kind`는 **실행 코드가 쓴 이름**이다
> (`attn_amplify` ↔ `kind: "spotlight"`). 값이 다를 뿐 같은 것을 가리킨다.

**공통 extra**

| 필드 | 무슨 값인가 |
|---|---|
| `mode` | `"steer"`(점수) / `"steer_generate"`(실제 생성) |
| `n_compliant` | 앞 코드 12개 중 준수한 개수. step6은 **전부 0**(절벽 바닥) |
| `target` | 지침이 요구한 표기(`"camel"`) |
| `tag` | 실행 묶음 라벨(`"cliff"`) |
| `layer` | **처방을 주입한 층.** Spotlight는 전 층에 걸리므로 `null` |

**값 조향(`value_add`)에만 있는 필드**

| 필드 | 무슨 값인가 |
|---|---|
| `strength` | 방향을 몇 배로 더했나(1·2·4·8). **세기가 셀수록 좋은 게 아니다** — §4-3 |
| `steer_source` | 방향을 무엇으로 만들었나. `"code_contrast"` = 코드 이름 자리의 camel 잔차 평균 − snake 잔차 평균 |
| `steer_vector` | 그 방향의 이력 — `n_samples`(쓴 표본 수) · `n_skipped`(이름 자리를 못 찾아 버린 수) · `vec_norm`(방향 벡터의 길이) · `layer`(뽑은 층). **무엇으로 만든 방향인지 파일만 보고 알 수 있게** 남긴다 |
| `hook_calls` | 잔차에 더하는 훅이 실제로 불린 횟수. **0이면 처방이 안 걸린 것** |
| `steer_from_layer` · `steer_layer` | **방향을 뽑은 층**(교차 주입 폴더에만 있다). `layer`와 같으면 "맞는 층 → 맞는 층", 다르면 **엉뚱한 층에 주입**한 조건(→ §3-4) |

**Spotlight(`attn_amplify`)에만 있는 필드**

| 필드 | 무슨 값인가 |
|---|---|
| `psi_target` | 지침 스팬이 차지하게 만들 **목표 어텐션 비중** ψ(0.1 · 0.3) |
| `span` | 어디를 밀어 올렸나. `"rule_word"`(규칙문 지시어만 — step5에서 값을 바꾼 자리와 정확히 같다) / `"instruction"`(지침 문장 전체 — 원 논문 정의) |
| `n_span_tokens` | 그 스팬의 토큰 수 |
| `attn_span_before` | **개입 전** 그 스팬의 어텐션 비중(이름을 쓰는 마지막 query 행 기준) |
| `attn_span_after` | **개입 후** 비중. 목표 ψ에 정확히 도달하지 않는 것은 원문 식(5)의 언더슛으로 **정상** |
| `attn_calls` | 갈아끼운 어텐션 함수가 불린 횟수. **0이면 예외로 죽는다** — 즉 저장된 파일은 전부 실제로 걸렸다는 뜻이고, 이것이 "구현 실패로 진 것 아니냐"는 반박을 막는 기록이다(→ §3-2) |

**생성 검증(`step6_steer-generate`)에만 있는 필드**

| 필드 | 무슨 값인가 |
|---|---|
| `compliance_rate`(metrics 최상위) | 생성된 이름이 목표 표기면 1.0 |
| `generated_text` | 조향을 건 채로 실제 생성한 원문 |
| `name` | 거기서 뽑아낸 함수 이름 |
| `notation` | 그 이름의 표기 판정(`camel`/`snake`/**`other`**). `RemoveDuplicates`(PascalCase)는 `other`다 |
| `compliant` | `notation == target` |
| `name_ok` | **이름이 멀쩡한가** — 식별자 형식 · 길이 2~40 · 같은 조각 반복 없음(`removeDuplicatesDuplicates`)을 모두 통과했나 |
| `name_reason` | 안 멀쩡하면 어디서 걸렸는지 |
| `name_length` | 이름 길이 |

### 1-2. 실제 파일 모양

**점수 회복 (`step6_steer`)**

```jsonc
"extra": {
  "mode": "steer", "method": "attn_amplify",     // none | value_add | attn_amplify
  "span": "rule_word", "psi_target": 0.1,        // Spotlight 전용
  "strength": null, "steer_source": null,        // 값 조향 전용
  "layer": null, "kind": "spotlight",
  "S_clean": 3.929, "S_base": -1.556, "S_int": -3.327,
  "recovery": -0.323,                            // ← 헤드라인
  "gap": 5.485, "undecidable": false,
  "attn_span_before": 0.00080,                   // ★ 개입이 실제로 걸렸는지
  "attn_span_after":  0.09053,
  "attn_calls": 64,                              // 0이면 예외였다
  "n_span_tokens": 3,
  "n_compliant": 0, "target": "camel", "tag": "cliff"
}
```

값 조향이면 `"method": "value_add"`, `"strength": 2`, `"layer": 25`,
`"hook_calls": …`, `"steer_vector": {"n_samples": 8, "n_skipped": 0, "vec_norm": …}`.

**생성 검증 (`step6_steer-generate`)**

```jsonc
"metrics": {
  "compliance_rate": 0.0,
  "extra": {
    "mode": "steer_generate", "method": "value_add", "strength": 4, "layer": 25,
    "generated_text": "def RemoveDuplicates(items):\n    …",
    "name": "RemoveDuplicates",
    "notation": "other",          // ← camel이 아니다!
    "compliant": false,
    "name_ok": true, "name_reason": "", "name_length": 16
  }
}
```

### 1-3. 읽는 순서

1. `method`(와 `strength` / `psi_target`)로 **어떤 처방의 어떤 세기**인지 확인한다.
2. **개입이 실제로 걸렸는지 먼저 본다** — Spotlight는 `attn_calls`·`attn_span_before/after`,
   값 조향은 `hook_calls`. 이 기록이 없으면 "효과 없음"과 "실험 안 됨"을 구분할 수 없다.
3. `recovery`를 본다. **단, 이 값만으로 처방의 성공을 말하지 않는다.**
4. **반드시 `step6_steer-generate`의 `compliant`·`notation`·`name_ok`와 함께 본다.**
   점수가 가장 높은 세기에서 실제 준수율이 0인 구간이 있다(→ §4-3). 점수는 위가 막혀
   있지 않아 세게 밀수록 계속 오르지만, 그 지점에서 모델은 이미 `RemoveDuplicates`처럼
   다른 표기로 넘어가 있다.
5. 집계: `python scripts/step6_summary.py --figs docs/step6/figures`

---

## 2. 무엇을 어떻게 쟀나

출발점은 **절벽 바닥** — 지침은 camelCase인데 앞 코드 12개가 전부 snake다(step1에서 준수율 0).
여기에 세 가지를 걸고 되살아나는 정도를 잰다.

| 방법 | 무엇을 하나 |
|---|---|
| **값 조향** | 그 층 잔차에 `세기 × (camel−snake 방향)` 더하기 [CAA·ITI] |
| **Spotlight** | 전 층·전 헤드에서 지침 스팬의 어텐션 비중을 ψ까지 끌어올림 |
| 무개입 | 하한선 |

- **준수 회복** = `(S_개입 − S_기준)/(S_깨끗 − S_기준)`. 1 = 문맥이 깨끗한 상태.
- 조향 방향은 **모델마다 자체 유도**한다(camel 이름 자리의 잔차 평균 − snake 자리 평균).
- Spotlight 스팬은 **두 종류**로 돌린다: 규칙문 지시어만 / 지침 문장 전체(원 논문 정의).
- 4모델 × 42묶음 × 13조건 = 2184개. 시드 42.
- **보조 측정:** 조향을 건 채로 실제 이름을 생성해 표기와 건전성을 본다(21묶음).

---

## 3. 결과

### 3-0. 그림이 네 묶음이다 — 어느 그림이 무엇인가

**두 종류의 실험이 섞여 있다는 것이 이 스텝 그림의 열쇠다.**

| 그림 묶음 | 몇 장 | 무엇으로 잰 값인가 | 세로축 | 왜 있나 |
|---|---|---|---|---|
| `explain_headline` | 1 (4모델) | **점수**(`steer`) | 회복률 | **이해용.** 값 조향 vs Spotlight 한 장 |
| `strength_<모델>` | 4 | **점수**(`steer`) | 회복률 | **논문용.** 세기·층별 |
| `explain_score_vs_real` | 1 (3모델) | **점수 + 실제 생성** 둘 다 | 좌: 비율 · 우: 회복률 | **이해용.** 둘이 어긋난다 |
| `name_health` | 1 (4모델) | **실제 생성**(`steer_generate`) | 비율 | 값 조향 세기별 준수율 + 이름 건전성 |
| **`dose_response`** | 1 (4모델) | **실제 생성 + 점수** | 이름 구성비 | **논문용 헤드라인.** 세기를 올리면 이름이 어디로 가나 |
| **`compliance_by_method`** | 1 (4모델) | **실제 생성** | 준수율 | **논문용.** 처방 비교를 준수율로 |
| **`score_vs_compliance`** | 1 (36점) | **점수 + 실제 생성** | 준수율 | **논문용.** 두 자가 어긋남을 한 장에 |

> ⚠️ **`회복률`과 `준수율`은 다른 자다. 섞어 읽으면 결론이 정반대가 된다.**
>
> | | 무엇 | 범위 | 어디서 |
> |---|---|---|---|
> | **회복률**(score recovery) | 두 후보 이름의 **로그확률 차이**가 바닥에서 천장 쪽으로 간 정도 | **1을 넘을 수 있다** | `step6_steer/` |
> | **준수율**(compliance) | 실제로 **생성된 이름**이 지침 표기였던 비율 | 0~1 | `step6_steer-generate/` |
>
> 이 스텝의 결론은 **둘이 어긋난다**는 것이다 — 세기 4에서 회복률은 최고, 준수율은 0.
>
> **그래서 `explain_headline`과 `strength_*`(회복률 기준)은 보조로 내리고,
> 논문 헤드라인은 `compliance_by_method`(준수율 기준)를 쓴다.**

---

### 3-1. 처방 비교 (준수 회복)

### 한눈에 보기 (이해용) — `explain_headline`

| 항목 | 이 그림에서 |
|---|---|
| **무엇을 재나** | 절벽 바닥에서 **값 조향**과 **Spotlight** 중 무엇이 점수를 되살리나 |
| **가로축** | 모델 4개 |
| **세로축** | **회복률** — 0 = 바닥 그대로, 1 = 깨끗한 문맥 수준, **1 초과 = 지나침** |
| **연회색 막대** `no intervention` | 무개입 = 정확히 0이라 **막대가 안 보인다**(하한선) |
| **진회색 막대** `Spotlight (attention, best)` | Spotlight 전체에서 **가장 높게 나온 값 하나** (아래 ⚠️) |
| **파란 막대** `Value steering @strength 2` | 값 조향, 세기 2, **맞는 층**(step3 봉우리). 42묶음 **평균** |
| **빨간 점선** `clean-context ceiling` | 회복률 1.0 = 깨끗한 문맥 수준 |
| **결과** | 파랑이 네 모델 모두 1.0을 넘고, 회색은 0.24~0.51 (Llama만 1.33으로 예외) |
| **뜻** | **값을 밀면 되고 어텐션을 키우면 안 된다.** 단 범용 모델(Llama)에서는 Spotlight도 작동한다 |

> ⚠️ **두 막대의 통계가 다르다 — 그래서 아래 표의 숫자와 회색 막대가 안 맞는다.**
> 파랑은 **42묶음 평균**이고, 회색은 `max(...)`로 뽑은 **개별 실행 최대값 하나**다
> (`make_explain_figures.py:217`). Spotlight를 유리하게 봐준 것이라 **우리 주장에는
> 보수적**이지만, 두 막대를 같은 자로 잰 값으로 읽으면 안 된다.
>
> | 모델 | 회색 막대(개별 최대) | 아래 표의 조건별 평균 최대 |
> |---|---|---|
> | Qwen | 0.244 | 0.175 |
> | DeepSeek | 0.316 | 0.040 |
> | Llama | 1.325 | 0.904 |
> | StableCode | 0.507 | 0.274 |
>
> **논문에 인용할 값은 아래 표(조건별 평균)** 이고, 이 그림은 이해용이다.

![이해용](figures/explain_headline.png)

### 논문용 그림 — `strength_<모델>` 4장

| 항목 | 이 그림에서 |
|---|---|
| **무엇을 재나** | 조향을 **얼마나 세게**, **어느 층에** 걸어야 하나 |
| **가로축** | **조향 세기** α (1·2·4·8). 눈금 간격이 균등하지만 값은 2배씩 뛴다 |
| **세로축** | 회복률 (위와 같은 자) |
| **진한 파란 선** `Value steering @L25` | **맞는 층**(step3에서 찾은 봉우리 층)에 주입 |
| **하늘색 선** `Value steering @early layer` | **엉뚱한 층**(초반)에 주입 — 대조 |
| **회색 파선** `Spotlight (best)` | Spotlight 최고값. 세기 축이 없어 **수평선**이다 |
| **빨간 점선** | 회복률 1.0 |
| **읽는 법** | 파랑이 회색 위인가(처방이 이기나) · **두 파란 선이 갈리나**(층이 중요한가) |
| **결과** | 파랑이 회색을 크게 넘는다. **그런데 두 파란 선이 세기 4~8에서 만난다** |
| **뜻** | 값을 미는 것은 통한다. **그러나 "맞는 층"이라는 우리 주장은 이 그림에서 지지되지 않는다**(→ §3-4·§4) |

**조향 세기별 회복 — 층별 대조 포함**

| Qwen2.5-Coder-3B | DeepSeek-Coder-6.7B |
|---|---|
| ![qwen](figures/strength_qwen.png) | ![deepseek](figures/strength_deepseek.png) |
| **Llama-3.2-3B (범용 — Spotlight도 작동)** | **StableCode-3B** |
| ![llama](figures/strength_llama.png) | ![stability](figures/strength_stability.png) |

| 방법 | Qwen | DeepSeek | Llama | StableCode |
|---|---|---|---|---|
| 무개입 | 0.000 | 0.000 | 0.000 | 0.000 |
| 값 조향 세기1 | **1.243** | 0.462 | **1.322** | 0.433 |
| 값 조향 세기2 | **1.643** | **1.920** | **1.385** | 2.006 |
| 값 조향 세기4 | 1.733 | 4.298 | 1.156 | 3.293 |
| 값 조향 세기8 | 1.612 | 2.805 | 0.783 | 2.303 |
| Spotlight 지시어 ψ0.1 | 0.002 | −0.366 | 0.424 | −0.232 |
| Spotlight 지시어 ψ0.3 | −0.125 | −0.051 | **0.904** | 0.274 |
| Spotlight 지침전체 ψ0.1 | 0.027 | 0.040 | 0.170 | 0.033 |
| Spotlight 지침전체 ψ0.3 | 0.175 | −0.174 | 0.077 | 0.038 |

판정 불가 0개(4모델 전부).

### 3-2. Spotlight는 실제로 걸렸다

"구현을 못해서 진 것"이라는 반박을 막기 위해 개입 전·후 어텐션 비중을 기록했다.

| 모델 | 조건 | 개입 전 | 개입 후 |
|---|---|---|---|
| Qwen | 지시어 ψ0.3 | 0.002 | **0.231** |
| DeepSeek | 지시어 ψ0.3 | 0.001 | **0.230** |
| Llama | 지시어 ψ0.3 | 0.001 | **0.231** |
| StableCode | 지시어 ψ0.3 | 0.002 | **0.231** |

**16개 조건 전부 비중이 올랐다.** 지시어 스팬은 100배 이상 올랐다(0.002 → 0.23).
목표치에 정확히 도달하지 않는 것은 원문 식(5)의 언더슛으로 정상이다.

### 3-3. 실제로 생성된 이름 — 이 스텝의 진짜 성적표

> 재현: `python scripts/step6_summary.py` → ③·③-b절.

#### ⚠️ 먼저 — 표기만 보면 안 된다

`camelCase`라는 문자열은 우리 분류기의 camel 정규식을 **통과한다.** 길이·형식·반복 검사
(`name_health`)도 전부 통과한다. 그런데 그건 **요청한 함수가 아니다** —
과제는 "중복을 제거하는 함수를 추가하라"였다.

실제로 Spotlight를 규칙문 지시어에 세게 걸었더니 모델이 **그 단어를 그대로 베껴 썼다**:

| 조건 | 나온 이름 |
|---|---|
| Llama `rule_word` ψ0.3 | **`camelCase` 17개** · `removeDuplicateCamelCase` 2 · 그 외 2 |
| DeepSeek `rule_word` ψ0.3 | `get_camera` 9 · **`camelCase` 8** · `generate_camelCase` 2 |

표기만 세면 Llama 준수율이 **1.000**으로 나온다. **완전한 가짜다.**

> **그래서 이 문서의 준수율은 「표기가 맞고 **+ 과제도 맞는**」 것만 센다.**
> 판정: 이름에 `remove`·`duplicat`·`dedup`·`uniq`·`distinct` 중 하나라도 들어 있으면
> 과제에 맞는 것으로 본다(`scripts/step6_summary.py`의 `on_task`).
> `xtract_unique_items`처럼 접두사가 붙어도 통과시키고, `camelCase`·`get_camera`·
> `udpate_list`·`ize`는 탈락이다.

#### 처방별 실제 준수율

| 모델 | 무개입 | 값조향 1 | 값조향 2 | 값조향 4 | 값조향 8 | Spotlight (4조건 최고) |
|---|---|---|---|---|---|---|
| **Qwen** | 0.000 | **1.000** | **1.000** | 0.000 | 0.000 | **0.000** |
| **DeepSeek** | 0.000 | **0.810** | 0.524 | 0.762 | 0.000 | **0.000** |
| **Llama** | 0.000 | **1.000** | **1.000** | **1.000** | 0.000 | **0.190** |
| StableCode | 0.000 | 0.000 | 0.095 | 0.000 | 0.000 | **0.000** |

**Spotlight는 4모델 어디서도 준수를 되살리지 못한다.** 16개 조건 중 15개가 정확히 0.000이고,
남은 하나(Llama `rule_word` ψ0.3 = 0.190)도 그 안의 2개는 `removeDuplicateCamelCase`로
지시어가 섞여 든 이름이다. **원 논문의 기본 스팬(`instruction` 문장 전체)은 전 모델 0.000이다.**

**값 조향은 세기 1~2에서만 작동한다.** 그 위로는 무너진다(Qwen 세기4에서 `RemoveDuplicates`,
DeepSeek·Llama 세기8에서 이름 자체를 못 만든다).

> **표기만 셌을 때와 달라진 곳** — 값 조향도 깎였다.
>
> | 조건 | 표기만 | **과제까지** |
> |---|---|---|
> | DeepSeek 세기2 | 0.952 | **0.524** |
> | DeepSeek 세기1 | 0.857 | **0.810** |
> | Llama SL rule ψ0.3 | 1.000 | **0.190** |
> | DeepSeek SL rule ψ0.3 | 0.381 | **0.000** |
>
> DeepSeek 세기2의 `ucFirst`·`udpateList`가 camel이라 준수로 세어지고 있었다.
> **DeepSeek의 최적 세기도 2가 아니라 1이다.**

### 한눈에 보기 (이해용) — `explain_score_vs_real`

| 항목 | 이 그림에서 |
|---|---|
| **무엇을 재나** | **같은 조건에서 점수와 실제 생성이 어긋나는가.** 이 스텝의 핵심 발견 |
| **가로축** | 조향 세기 (0·1·2·4·8) |
| **왼쪽 세로축** `rate` | 0~1 비율 — 파란 선과 초록 선이 쓴다 |
| **오른쪽 세로축** `score recovery` (주황) | **회복률. 눈금이 모델마다 다르다** — 높이를 모델끼리 비교하면 안 된다 |
| **파란 실선** `ACTUAL compliance` | **실제로 생성시켜서** 나온 이름이 지침대로 camelCase였나. **왼쪽 축.** ← 진짜 성적표 |
| **초록 파선** `name still valid` | 그 이름이 **말이 되는 식별자**인가(형식·길이 2~40·같은 조각 반복 없음). **왼쪽 축** |
| **주황 선** `preference-score recovery` | **생성을 안 하고** 두 후보의 로그확률만 재서 낸 회복률. **오른쪽 축** — 눈금이 다르다 |
| **핵심** | **파랑·초록은 "실제로 쓴 이름", 주황은 "쓰기 전 선호".** 이 셋이 따로 논다는 것이 이 그림의 전부다 |
| **판이 3개인 이유** | **StableCode를 뺐다.** 이유 두 가지 — ① 준수율이 어느 세기에서도 ≤0.143이라 "점수↑ 준수↓"의 **대비가 안 보인다** ② 더 중요한 것: **무개입(세기 0)에서부터 이름이 깨져 있다**(`xtend_remove_duplicates`·`xtra_remove_duplicates`). 조향 탓이 아니라 `def ` 강제 접두와 이 모델 토크나이저의 궁합 문제라 **처방을 평가할 수조차 없다**(→ §4-5) |
| **주의** | **논문용 `name_health`에는 StableCode가 들어 있다**(4모델). 이해용만 3판인데 그림 자체에는 그 표시가 없으니, 인용할 때 이 차이를 밝힌다 |
| **읽는 법** | **파랑과 주황이 같은 방향으로 움직이나**를 본다 |
| **결과** | 세기 4에서 **주황은 최고인데 파랑은 0으로 무너진다**(Qwen) |
| **뜻** | **점수만 보면 처방이 성공한 것처럼 보이는 구간이 있다.** 그래서 생성을 따로 확인해야 한다 |

![이해용](figures/explain_score_vs_real.png)

파랑(실제 준수율)이 세기 4에서 무너지는데 주황(선호 점수 회복)은 그때 최고다.
**점수만 보면 처방이 성공한 것처럼 보이는 구간이 있다.**

#### 두 자의 관계를 숫자로 — 짝지은 20개 조건

> 재현: `python scripts/step6_summary.py` → ③-b절.
> 회복률은 `step6_steer/`, 준수율은 `step6_steer-generate/`에서 **같은 조건끼리 짝지었다.**

> ⚠️ 준수율은 **`on_task` 판정을 거친 값**이다 — 표기만 맞고 과제를 벗어난 출력
> (`camelCase`를 그대로 베끼는 등)을 준수로 세지 않는다. 판정 도입 전 값과 다르다.

| 모델 | 조건 | 회복률 | 실제 준수율 |
|---|---|---|---|
| **Qwen** | 무개입 | 0.000 | 0.000 |
| | 세기 1 | 1.243 | **1.000** |
| | 세기 2 | 1.643 | **1.000** |
| | **세기 4** | **1.733** ← 최고 | **0.000** ← 바닥 |
| | 세기 8 | 1.612 | 0.000 |
| **DeepSeek** | 무개입 | 0.000 | 0.000 |
| | **세기 1** | **0.462** ← 천장의 절반도 안 됨 | **0.810** ← 가장 높다 |
| | 세기 2 | 1.920 | 0.524 |
| | **세기 4** | **4.298** ← 최고 | 0.762 |
| | 세기 8 | 2.805 | **0.000** |
| **Llama** | 세기 1~4 | 1.156~1.385 | **1.000** |
| | 세기 8 | 0.783 | 0.000 |
| | **SL 규칙어 ψ0.3** | **0.904** ← Spotlight 최고 | **0.190** ← 과제 맞음도 0.190 |
| **StableCode** | **세기 1** | **0.433** ← DeepSeek 세기1과 거의 같다 | **0.000** ← 정반대 |
| | **세기 2** | **2.006** ← 4모델 점수 1위 | **0.095** ← 준수 꼴찌 |
| | 세기 4 | 3.293 | 0.000 |

**DeepSeek 안에서 회복률과 준수율의 순위가 정반대다** — 회복률은 세기 1 < 2 < 4로 오르는데
준수율은 세기 1(0.810)이 가장 높고 세기 2(0.524)가 가장 낮다.
**StableCode는 점수 1위인데 준수 꼴찌다.**

**구간을 나눠 봐야 한다 — 전체 상관만 보면 오해한다.**

| 구간 | n | 상관계수 r | 뜻 |
|---|---|---|---|
| 약한 조향 (세기 ≤2) | 28 | **+0.72** | 같이 오르는 것처럼 보인다 |
| 센 조향 (세기 ≥2) | 12 | **−0.09** | 관계가 끊긴다 |
| **전체** | 36 | **+0.39** | **두 구간이 상쇄돼 평평해 보인다** ← 이 숫자만 인용하면 안 된다 |

> ⚠️ 약한 구간의 `+0.72`는 **Spotlight 조건 16개와 원점 4개가 만든 값**이다. 그 20개는
> 회복률도 준수율도 대부분 0 근처에 몰려 있어(둘 다 0인 점이 다수) 직선을 끌어당긴다.
> **0이 아닌 값끼리의 관계가 아니다.** "약한 구간에서는 점수를 믿어도 된다"고 말할 근거가 **없다.**

**핵심은 상관계수가 아니라 흩어짐이다.**

| 회복률 | n | 실제 준수율 **최소** | **최대** |
|---|---|---|---|
| < 1.0 | 19 | 0.000 | **0.810** |
| ≥ 1.5 | 9 | **0.000** | 1.000 |

**어느 회복률 구간에서도 준수율이 0부터 1까지 흩어진다.** 가장 날카로운 대비:

> DeepSeek 세기1 (**회복률 0.462 → 준수율 0.810**) vs
> StableCode 세기1 (**회복률 0.433 → 준수율 0.000**)
> — **거의 같은 회복률에서 준수율이 정반대다.**

**따라서 실패는 한쪽 방향이 아니다.** "점수가 높으면 못 믿고, 낮으면 믿어도 된다"가
**아니라**, 높든 낮든 준수율을 말해 주지 않는다. 이 사실이 §4-(3)의 근거이고,
**점수만으로 처방을 비교하는 서술을 쓸 수 없게 만든다**(→ §5 「이 스텝이 다루지 않는 것」).

### 논문용 그림 — `name_health`

| 항목 | 이 그림에서 |
|---|---|
| **무엇을 재나** | 위 그림에서 **점수를 빼고**, 실제 생성만 4모델 한 장에 |
| **판 2개** | **왼쪽 = 준수율 · 오른쪽 = 이름 건전성.** 예전에는 한 축에 실선·파선으로 겹쳐 8개 선이 뒤엉켰다 |
| **가로축** | 조향 세기 (0·1·2·4·8). **눈금 간격이 균등**하다(값은 2배씩 뛴다) |
| **세로축** | 비율 0~1 (두 판이 같은 눈금) |
| **왼쪽 판** `Compliance (correct notation + on task)` | 생성된 이름이 **camel이고 + 과제(중복 제거)의 함수**인 비율 |
| **오른쪽 판** `Name intact` | 이름이 **깨지지 않은** 비율 — 식별자 형식 · 길이 2~40 · 같은 조각 반복 없음 |
| **색** | Qwen(파랑) · DeepSeek(주황) · Llama(초록) · StableCode(빨강) |
| **읽는 법** | **두 판을 나란히 본다.** 왼쪽만 떨어지고 오른쪽이 1.0이면 "이름은 멀쩡한데 표기를 안 지킨" 것(Qwen 세기4의 `RemoveDuplicates`). 둘 다 떨어지면 생성 자체가 망가진 것(세기8) |
| **결과** | 세기 1~2에서 실선이 천장(Qwen·Llama 1.00 · **DeepSeek 0.81**). **세기 4~8에서 무너진다.** Qwen은 세기 4에서 파선이 1.00인데 실선만 0 |
| **주의** | **이 그림은 값 조향과 무개입만** 그린다. Spotlight는 세기 축이 없어 얹을 수 없다 — 처방 비교는 `compliance_by_method`를 본다 |
| **뜻** | 처방에는 **작동 구간이 있다**(세기 1~2). Qwen 세기 4의 실패는 이름이 깨져서가 아니라 `RemoveDuplicates`처럼 **camel도 snake도 아닌 표기**가 돼서다 |

![이름 건전성](figures/name_health.png)

### 3-3a. 세기를 올리면 이름이 어디로 가나 — `dose_response` (논문 헤드라인)

| 항목 | 이 그림에서 |
|---|---|
| **무엇을 재나** | 조향 세기를 올릴 때 **생성된 이름이 어떤 종류로 바뀌는가**. step6 전체를 한 장에 |
| **판 4개** | 모델 4개 |
| **가로축** | 조향 세기 (0·1·2·4·8). **0 = 무개입** |
| **왼쪽 세로축** | 생성된 이름 21개의 **구성비**(막대는 항상 합이 1) |
| **빨강** `violating (snake)` | `remove_duplicates` — 지침을 어긴 표기 |
| **파랑** `compliant (camel)` | `removeDuplicates` — **지침대로 + 과제도 맞음** |
| **주황** `overshoot (Pascal)` | `RemoveDuplicates` — camel 방향으로 **지나쳐** 첫 글자까지 대문자가 됨 |
| **회색** `broken / none` | `ize`·`_Printachticker`·이름 없음 등 |
| **검은 선 (오른쪽 축)** | **점수 회복률.** 눈금이 왼쪽과 다르다 |
| **읽는 법 ①** | **빨강이 언제 사라지나** — 세기 1에서 즉시 사라지면 개입이 실제로 작동한 것 |
| **읽는 법 ②** | **파랑이 어느 세기까지 유지되나** — 그게 그 모델의 최적 용량 |
| **읽는 법 ③** | **검은 선과 파랑이 같이 움직이나** — 안 움직이면 점수를 용량계로 못 쓴다 |

![용량-반응](figures/dose_response.png)

**세 가지가 한 번에 보인다.**

**(가) 인과다.** 무개입에서 빨강이 100%인데, **세기 1을 걸자마자 4모델 전부 빨강이 사라진다.**
개입이 행동을 실제로 바꾼다.

**(나) 실패가 아니라 지나침이다.** Qwen 세기 4~8은 빨강으로 **되돌아간 게 아니라** 주황으로 갔다 —
`remove_duplicates` → `removeDuplicates` → `RemoveDuplicates`. **camel 방향으로 더 간 것**이고,
첫 글자까지 대문자가 돼 우리 분류기가 `other`로 셀 뿐이다. 방향은 끝까지 맞았다.

**(다) 점수는 용량계가 아니다.** Qwen은 파랑이 사라진 세기 4에서 검은 선이 **최고점**이다.
DeepSeek은 회색이 100%가 된 세기 8에서도 회복률 2.8이다.
**이름이 다 망가진 뒤에도 점수는 높다.**

> **모델마다 견디는 용량이 다르다** — Llama는 세기 4까지, Qwen은 2까지, DeepSeek은 1~2까지.
> StableCode는 파랑이 거의 안 나온다(→ §4-5, 측정 장치 문제).

### 3-3b. 논문 헤드라인 — `compliance_by_method`

| 항목 | 이 그림에서 |
|---|---|
| **무엇을 재나** | 처방 비교를 **실제 준수율로**. `explain_headline`이 같은 비교를 점수로 하는데, 점수는 준수율을 대변하지 않으므로(§3-3) **논문에는 이쪽이 맞다** |
| **가로축** | 모델 4개 |
| **세로축** | **실제 준수율** — 표기가 맞고 **+ 과제도 맞는** 이름의 비율 |
| **연회색 막대** `no intervention` | 무개입. 4모델 전부 0.000이라 **막대가 안 보인다**(하한선) |
| **파란 막대** `Value steering (best α)` | 값 조향, 모델별 **최적 세기** |
| **진회색 막대** `Spotlight (best span·ψ)` | Spotlight, 4조건 중 **최고**. Llama만 보이고 나머지는 0 |
| **오차 막대** | 21묶음 95% 신뢰구간 |
| **결과** | 파랑 1.00 / 0.81 / 1.00 / 0.10 · 회색은 Llama 0.19 말고 전부 **0.000** |
| **뜻** | **값 조향은 통하고 Spotlight는 안 통한다** — 판정 가능한 세 모델 전부에서 |

![처방 비교](figures/compliance_by_method.png)

### 3-3c. 두 자가 어긋난다 — `score_vs_compliance`

| 항목 | 이 그림에서 |
|---|---|
| **무엇을 재나** | 회복률과 실제 준수율을 **같은 조건끼리 짝지어** 한 점으로 |
| **가로축** | 점수 회복률 (음수부터 4.3까지) |
| **세로축** | 실제 준수율 (0~1) |
| **모양** | ● 무개입 · ▲ 값 조향 · □ Spotlight |
| **색** | 모델 4개 |
| **빨간 세로 점선** | 회복률 1.0 (= 깨끗한 문맥 수준) |
| **읽는 법** | 점들이 **대각선을 이루면** 두 자가 일치하는 것이다. 실제로는 흩어진다 |
| **결과** | 회복률 1.6~4.3인데 준수율 0인 점이 여럿 · 회복률 0.46인데 준수율 0.81인 점도 있다 |
| **뜻** | **회복률로 처방의 성패를 가릴 수 없다.** 이 그림이 §4-(3)의 근거다 |

![점수 대 준수율](figures/score_vs_compliance.png)

### 3-4. 방향은 층을 가리지 않는다

맞는 층에서 뽑은 방향을 엉뚱한 층(초반)에 넣어 봤다.

| 모델 | 세기 | 맞는층→맞는층 | 엉뚱층→엉뚱층 | 맞는층→엉뚱층 |
|---|---|---|---|---|
| Qwen | 2 | 1.643 | 1.436 | **1.827** |
| DeepSeek | 4 | 4.298 | 2.690 | 2.475 |
| Llama | 2 | 1.385 | 1.330 | 1.251 |
| StableCode | 1 | 0.433 | 0.201 | **3.845** |

**어느 조합도 일관되게 우세하지 않다.** 교차 주입이 더 높은 경우도 많다.

---

## 4. 해석

> **모든 수치는 「표기가 맞고 + 과제도 맞는」 실제 생성 준수율이다**(→ §3-3).
> 회복률은 이 절의 근거가 아니다.

**(1) 어텐션을 키우는 접근은 준수를 되살리지 못한다.**
Spotlight 16개 조건(4모델 × 스팬 2 × ψ 2) 중 **15개가 정확히 0.000**이다.
원 논문의 기본 스팬(`instruction` 문장 전체)은 **전 모델 0.000**이다.
어텐션 비중은 실제로 올랐다(0.002 → 0.23, 100배 이상, §3-2) — **"구현 실패로 졌다"고
돌릴 수 없다.** step4에서 모델이 이미 지침을 충분히 본다는 것을 관측했고,
여기서 더 보게 만들어도 소용없음을 **실제 생성으로** 확인했다.

**(2) 값을 밀면 준수가 되살아난다 — 단 세기 1~2에서만.**
절벽 바닥 0.000에서 **Qwen 1.000 · Llama 1.000 · DeepSeek 0.810**까지 올라간다.
**최적 세기는 모델마다 다르다** — Qwen·Llama는 1~2, DeepSeek은 1이다.

**(2-b) 세게 밀어 준수율이 떨어지는 것은 실패가 아니라 지나침이다 — 인과의 증거다.**
"들쭉날쭉하니 인과가 아니지 않느냐"는 반문이 나올 수 있다. 아니다. **어디로 갔는지**를 보면
방향은 끝까지 맞았다(→ §3-3a `dose_response`).

**생성된 이름 21개가 어느 표기였나 — 구성비**

| 세기 | | snake (위반) | camel (준수) | other | 대표 이름 |
|---|---|---|---|---|---|
| **0** (무개입) | Qwen | **1.00** | 0.00 | 0.00 | `remove_duplicates` |
| | DeepSeek | **1.00** | 0.00 | 0.00 | `remove_duplicates` |
| | Llama | **1.00** | 0.00 | 0.00 | `remove_duplicates` |
| **1** | Qwen | 0.00 | **1.00** | 0.00 | `removeDuplicates` |
| | DeepSeek | 0.14 | **0.86** | 0.00 | `removeDuplicates` |
| | Llama | 0.00 | **1.00** | 0.00 | `removeDuplicates` |
| **2** | Qwen | 0.00 | **1.00** | 0.00 | `removeDuplicates` |
| | DeepSeek | 0.00 | **0.95** | 0.05 | `removeDuplicates` |
| | Llama | 0.00 | **1.00** | 0.00 | `removeDuplicates` |
| **4** | Qwen | 0.00 | 0.00 | **1.00** | `RemoveDuplicates` |
| | DeepSeek | 0.00 | **0.76** | 0.24 | `removeDuplicates` |
| | Llama | 0.00 | **1.00** | 0.00 | `removeDuplicates` |
| **8** | Qwen | 0.00 | 0.00 | **1.00** | `RemoveDuplicates` |
| | DeepSeek | 0.00 | 0.00 | **1.00** | 이름 없음 |
| | Llama | 0.00 | 0.00 | **1.00** | `_Printachticker` |

**snake 칸을 세로로 보면 된다.** 무개입에서 1.00인데 **세기 1부터 끝까지 0.00이다**
(DeepSeek 세기1의 0.14만 예외). StableCode는 §4-5의 측정 장치 문제로 뺐다.

세기가 오르면서 Qwen이 지나간 길:

```
   세기 0 :  remove_duplicates    ← snake (위반)
   세기 1 :  removeDuplicates     ← camel (준수) ✅
   세기 2 :  removeDuplicates     ← camel (준수) ✅
   세기 4 :  RemoveDuplicates     ← 첫 글자까지 대문자. camel 방향으로 **더 갔다**
```

- **무개입에서 위반이 100%인데, 세기 1을 걸자마자 4모델 전부 위반이 사라진다.**
  개입이 행동을 실제로 바꾼다.
- **세기 4에서 위반으로 되돌아간 것이 아니라 camel을 지나쳐 갔다.** `RemoveDuplicates`는
  camel 방향의 **과잉**이지 반대 방향이 아니다. 우리 분류기가 `other`로 셀 뿐이다.
- 어느 세기에서도 snake가 다시 나오지 않는다.

**용량-반응이다.** 안 밀면 안 낫고, 적당히 밀면 낫고, 과하게 밀면 부작용이 난다.
견디는 용량은 모델마다 다르다(Llama 세기 4까지, Qwen 2까지, DeepSeek 1~2).

**(3) 선호 점수는 준수율을 대변하지 않는다 — 이 스텝의 방법론적 발견.**
짝지은 36개 조건에서 전체 상관 r = +0.39이고, **회복률이 1 미만인 구간에서도 준수율이
0.000~0.810으로 흩어진다**(→ §3-3). 양방향으로 깨진다:

| 조건 | 회복률 | 실제 준수율 |
|---|---|---|
| Qwen 세기4 | **1.733** (최고) | **0.000** |
| DeepSeek 세기1 | **0.462** (낮음) | **0.810** |
| StableCode 세기1 | 0.433 (거의 같음) | 0.000 |

> **점수만 보고 처방의 성공도 실패도 주장할 수 없다.** 높으면 못 믿고, 낮아도 못 믿는다.
> 회복률에 남은 쓸모는 **개입이 걸리기는 했는지** 확인하는 것뿐이다.

**(3-b) 표기만 세는 채점도 못 믿는다 — 이번에 발견했다.**
Spotlight를 지시어에 세게 걸면 모델이 **그 단어를 함수 이름으로 베껴 쓴다**
(`def camelCase(...)`, Llama 21개 중 17개). `camelCase`는 camel 정규식을 통과하고
길이·형식·반복 검사도 통과해 **준수 1.000으로 세어졌다.** 과제 적합성까지 봐야
0.190으로 내려간다. **조향 논문이 표기 일치만 보고하면 이 함정에 그대로 빠진다.**

**(4) 방향은 층 특이적이지 않다.**
초반 층에 밀어도, 맞는 층 방향을 초반 층에 넣어도 비슷하게 작동한다.
잔차 스트림은 층을 타고 누적되므로 앞쪽에 더한 성분이 뒤까지 전달되는 것이 자연스럽다.
**step3의 "표기 정보가 특정 층에 국소화된다"는 관측 결과와 별개**이며, 처방 단계에서는
층 선택이 결정적이지 않다.

**(5) StableCode는 이 실험에서 처방이 성립하지 않는다.**
어느 조건에서도 0.095를 넘지 않는다. 다만 **무개입에서도 `xtend_remove_duplicates`처럼
접두사가 붙은 이름이 나온다.** 교사강제 접두 `"def "`와 이 모델 토크나이저의 조합에서
생성이 어긋나는 것으로 보인다 — 조향의 실패라기보다 **측정 장치의 문제**일 가능성이 크다.
이 모델의 생성 결과는 해석하지 않는다.

**(6) 값 조향이 Spotlight보다 낫다 — 판정 가능한 세 모델 전부에서.**
Qwen 1.000 vs 0.000 · Llama 1.000 vs 0.190 · DeepSeek 0.810 vs 0.000.
점수로만 보면 Llama에서 Spotlight가 0.904까지 올라가 대등해 보이지만, **실제로 생성시키면
그 조건에서 나오는 이름이 `camelCase`다.** 점수를 믿었으면 결론이 뒤집힐 뻔했다.

---

## 5. 이 스텝이 다루지 않는 것

- **과제 적합성 판정이 단어 매칭이다.** 이름에 `remove`·`duplicat`·`dedup`·`uniq`·`distinct`가
  들어 있으면 통과시킨다. `removeDuplicateCamelCase`처럼 **지시어가 섞여 든 이름도 통과**하므로,
  보고 값은 **상한**이다. Llama Spotlight 0.190 중 2개가 그런 이름이다.
- **한 이름 한 쌍**에 대한 처방이다. `removeDuplicates` vs `remove_duplicates` 고정.
- **절벽 바닥 한 지점**에서만 잰다(위반 12/12). 중간 지점은 다루지 않는다.
- 조향 방향은 **step3와 같은 데이터**에서 유도했다. 독립 표본에서의 일반화는 확인하지 않았다.
- Spotlight의 자유도(전 쿼리 위치 적용·헤드별 독립 ψ)는 원문에 명시가 없어 우리가 정했다.
- StableCode의 생성 경로 이상(§4-5)은 원인을 규명하지 않았다.
- 세기 격자가 성기다(1·2·4·8). 최적점이 1과 2 사이일 수 있다.
- **생성 검증은 21묶음**이다(점수는 42묶음).
