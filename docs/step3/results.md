...
| 그림 묶음 | 몇 장 | 무엇을 보여주나 | 왜 있나 |
|---|---|---|---|
| `explain_bubble` | 1장 (4모델 한 장) | 봉우리 층 **한 점**에서, 정보 있는 값으로 덮었을 때 vs 없는 값으로 덮었을 때 | **이해용.** 이 스텝의 결론을 한 장으로 |
| `bubble_<모델>` | 4장 | 층별로 **원래 값 · 그냥 덮어도 나오는 값 · 표기 때문에 생긴 값** 세 곡선 | 논문 그림의 숫자가 **어떻게 깎여 나왔는지**를 보이려고 |
| `donors_<모델>` | 4장 | 층별로 **덮어넣는 값 3종의 원래 값** 곡선 | 세 조건이 각각 어디에 있는지 — 빼기 전 raw 상태 |
| `code_causality_<모델>` + `key_vs_value` | 4+1장 | **표기 때문에 생긴 값만** (내용/어텐션/둘다) | **논문용.** 최종 결과 |
...
| **가로축** | 몇 번째 층을 덮었나 |
| **세로축** | 되돌림 — **`raw, not net`**, 즉 아무것도 빼지 않은 값 |
