...
>
> **방법론·코드 상세:** `docs/step3/방법론_코드와개념.md` (트랜스포머 개념 · KV 치환 해부 · 코드 흐름 · 한계)
...
2. **내용(Value)이 어텐션(Key)보다 유의하게 더 되돌린다** — 4모델 모두, 신뢰구간이 0을 넘지 않음.
   ~~그리고 **어텐션을 내용 위에 더해도 안 늘어남**(둘다 ≈ 내용만) → 내용이 충분, 어텐션은 잉여.~~
   `[정정]` **철회.** 이 등식은 통제를 빼지 않은 원값에서만 성립한다(0.535 vs 0.536).
   표기 순효과로는 **둘다가 내용만의 절반 이하**다(qwen 0.117 vs 0.253 — 4모델 모두, 신뢰구간 안 겹침).
   어텐션을 얹으면 표기 신호가 **깎인다.** 이는 Key 치환의 RoPE 위상 왜곡과 같은 지문이므로
   진단 전까지 어느 쪽인지 확정할 수 없다. `docs/step3/결과_통제보정.md` §3 참조.
...
- deepseek은 되돌림 자체가 작고(0.20) 다른 셋보다 봉우리가 낮다.
  `[정정]` "여러 층에 흩어져"는 통제를 빼기 전 곡선의 인상이다. 표기 순효과로 보면
  deepseek도 **봉우리 절반 이상인 층이 L20 하나뿐**이다 — 국소화는 4모델 모두 같다.
  또한 모델 간 **크기 비교**는 덮은 자리 수가 달라(평균 12.7 vs 22.5) 그대로 하면 안 된다.
