...
통과시킨 것**이다. 프롬프트 원문과 측정 절차는 [`방법론.md` §2-1·2-2](방법론.md)에 있다 —
**프롬프트가 몇 개 만들어지고 그중 어느 것을 채점하는지**가 헷갈리면 거기부터 본다. 그 파일에는 실험 문맥에 **한 번도 안 나온** 함수 6개가 들어 있고, 전부 한
...
step5(지침 쪽 인과)도 **이제 전 층에서 잰다.** 예전에는 봉우리 층 한 곳에서만 재서
층별 곡선을 만들 수 없었고 봉우리 층도 "고른 것"이 아니라 "뺄 수 있는 유일한 층"이었으나,
통제를 전 층으로 다시 돌려 그 비대칭이 없어졌다(→ [`../step5/results.md`](../step5/results.md) §2-3·§3-1).

한때 이런 근거를 세웠었다 — **step3에서는 원래 값의 봉우리 층과 표기 때문에 생긴 값의
봉우리 층이 네 모델 모두 일치하므로**(L25·L20·L15·L18), step5가 한 층만 재고도 봉우리를
맞게 짚었을 것이라는 **가정**이었다. **이 가정은 이제 직접 확인됐다** — 전 층에서 재 보니
네 모델 모두 전에 쓰던 그 층이 실제 봉우리였다(→ step5 results §3-2).
