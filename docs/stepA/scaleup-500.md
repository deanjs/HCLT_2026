...
> 모델이 **읽는** 코드에 규약 준수 예시가 몇 개 남았느냐에 따라,
> 모델이 **새로 쓰는** 코드의 준수율이 어떻게 변하는가.

특히 두 가지를 본다.

- **절벽(cliff):** 준수 예시가 줄어도 한동안 버티다가, 어느 지점에서 준수율이 **급락**하는가? (매끄러운 하락이 아니라 무너지는가)
- **자기증폭(self-amplification):** 모델이 첫 함수를 위반하면, 그 뒤 함수들로 위반이 **연쇄**되는가? (자기 출력이 다시 입력이 되어 굴러가는가)
