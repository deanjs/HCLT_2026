...
### 그림 한 장이 전부다 — `kv_phase_shrink`

| 항목 | 이 그림에서 |
|---|---|
| **무엇을 재나** | 평균 덮어쓰기가 **어텐션 쪽(Key)을 내용 쪽(Value)보다 더 망가뜨리나** |
| **판이 8개인 이유** | **가로 4칸 = 모델 4개**(deepseek·llama·qwen·stability), **세로 2줄 = 덮은 대상**(위 = 코드 이름 / 아래 = 지침 지시어) |
| **가로축** | **몇 번째 층인가**. 모델마다 층수가 달라 축 길이가 다르다(28~36) |
| **세로축** | **줄어듦 비율** = ‖조각들의 평균‖ ÷ ‖조각 길이‖의 평균. **1 = 손해 없음 · 낮을수록 뭉개짐** |
| **회색 선** `Key (has rotation)` | 어텐션 쪽. **위치 회전(RoPE)이 걸리는 경로** — 우려의 대상 |
| **파란 선** `Value (no rotation)` | 내용 쪽. 회전이 안 걸린다 → **회전을 뺀 순수 평균 손실의 기준선** |
| **점선(1.0)** | 손해가 전혀 없는 상태 |
| **읽는 법** | **회색이 파랑보다 아래면 문제**(Key가 더 망가짐). 위면 우려 기각 |
| **결과** | **8개 판 전부에서 회색이 파랑 위에 있다.** 회색 0.94~0.96, 파랑 0.79~0.89 |
| **뜻** | Key는 거의 안 줄어든다. **"어텐션 쪽 되돌림이 0"은 개입이 망가져서가 아니다** |

> **왜 Value가 기준선인가.** 서로 다른 조각을 평균 내면 **회전이 없어도** 길이는 줄어든다.
> Key만 재면 "0.95가 나쁜 값인가"를 판단할 수 없다. 회전이 안 걸리는 Value가
> "평균만으로 얼마나 줄어드는가"를 알려주므로, **둘의 차이가 곧 회전 탓 추가 손해**다.
> 차이가 **양수**로 나왔다 = 회전 탓 추가 손해가 **없다**.
