...
대응: **RQ2** — 매개 신호의 성격(형태·경로·층)이 모델 간에 보존되는가. → 보존되면 step5 방법론(Value 경로 스티어링)이 모델 일반적으로 성립.
...
## 범위 — step1만 반복, 같은 데이터셋 (stepA 절벽은 제외)

- step2는 **step1의 기제 실험(개입 스윕 + v 코사인)만** 다른 모델에 반복한다. **stepA의 행동 절벽(n=4/3/2/1/0 준수율 곡선)은 반복하지 않는다** — 그건 RQ1이고, step2는 RQ2(신호의 모델 간 보존)의 확장이다.
- **데이터셋은 step1과 완전히 동일**하게 고정한다(POOL n=0, pos-camel-weak, 같은 seed). 데이터까지 바꾸면 차이가 모델 탓인지 데이터 탓인지 분리 불가 → **모델만 변수로 남긴다.** (문자열 데이터는 같아도 모델마다 토크나이저가 다르므로 정렬률 sanity로 확인.)
- **전제 확인 = mini-RQ1.** step1의 sanity였던 `S_깨끗 > S_위반`이 각 모델에서 성립해야 스윕이 의미를 갖는다(위반 선행이 선호를 실제로 뒤집는가). 불성립 시 그 모델은 **"현상 없음"으로 그대로 기록**하고 스윕을 강행하지 않는다(§4). 전체 RQ1 절벽의 모델별 재현은 step2 범위 밖.
...
| **Value 우세 보존** | "Value 경로"가 일반. step5 개입을 Value로 |
