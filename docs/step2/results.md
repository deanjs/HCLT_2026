...
같은 관측 하나(168개 파일)를 **다섯 가지로 잘라 그린 것**이다. 새 실험이 아니다.
**다섯 묶음 전부 가로축이 "몇 번째 층인가"**이고, 다른 것은 **세로축과 그리는 선**뿐이다.

| 그림 묶음 | 몇 장 | 세로축 | 그리는 선 | 답하는 질문 |
|---|---|---|---|---|
| `spans_<모델>` | 4 | 토큰당 어텐션 | **구간 5종 전부** | 프롬프트의 **어디**를 보나 |
| `attention_<모델>` | 4 | 토큰당 어텐션 | 코드 camel · snake · 지침(참고) | 코드 두 무리 중 **어느 쪽**을 더 보나 |
| `av_<모델>` | 4 | 토큰당 ‖a·v‖ | 코드 camel · snake | 어느 쪽이 **더 크게 기여할 수 있나** |
| `vnorm_<모델>` | 4 | 토큰당 ‖v‖ | 코드 camel · snake | 내용 **자체의 크기**가 다른가 |
| `gap_<모델>` | 4 | snake − camel **차이** | 토큰당(실선) · 합(점선) | **정규화가 결론을 바꾸나** |
