...
**같은 단어쌍에서 두 표기를 만든다.** 그래서 **의미와 어휘 정체성**이 맞춰진다.

> ⚠️ **길이·토큰 수·말뭉치 빈도까지 통제되는 것은 아니다.** `format_matrix`는 밑줄 때문에
> 한 글자 길고 조각도 하나 더 많다. 두 표기의 사전학습 빈도도 다르다.
> 통제됐다고 말할 수 있는 것은 **어휘 정체성과 의미**까지다.
...
의도된 통제다 — 함수 **본문**이 전부 같으므로 내용 차이가 안 낀다.

> 다만 12개 함수의 **이름 자체는 서로 다르다**(`parseHeader`, `buildToken`, …).
> "오직 표기만 다르다"가 성립하는 것은 **같은 이름의 camel판 ↔ snake판 짝** 안에서다.
...
이름·본문(`return value`)은 두 언어에서 같다.

> ⚠️ **언어 효과를 제거한 것이 아니다.** 구문·토크나이저·언어별 사전학습 prior가 모두 다르다.
> 이 조건은 **"언어를 바꿔도 같은 현상이 나오나"라는 강건성 비교**이지,
> "표기 관습만 남기고 언어를 통제했다"가 아니다.
