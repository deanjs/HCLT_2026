...
**왼쪽 — 코드 이름 토큰(초록=camel, 주황=snake):** 다른 층 평균(~0.021)에 머물다 **L25에서 0.147로 급등**(약 **7배**)하고 곧바로 잦아든다. 모델이 코드 이름 토큰을 **바로 L25에서 집중해 읽는다**는 뜻. stepC가 개입 층으로 L25를 고른 근거가 **500개 이름에서도** 재현된다.

**오른쪽 — 지침 구간(보라, 따로):** 지침 어텐션은 층별로 높고 변동이 크다. 단 이는 지침 토큰이 37개로 많아 **합**이 큰 것이라, 코드 토큰의 L25 급등과 **스케일이 달라 같은 그림에 겹치면 급등이 묻힌다** → 그래서 분리했다. 지침은 층별 절대량이 아니라 **조건 간 비교(②)·토큰당 비교(③)** 로 봐야 한다.
