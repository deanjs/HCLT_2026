...
### 재현 되는 것: 지시어는 **대부분 모델에서 뚜렷이 읽힌다** (3/4) ✅

**⚠️ 정규화 주의:** raw per-token 값은 모델 간 비교 불가(토크나이저·seq_len·head수·어텐션 날카로움이 달라 기준선이 다름). **균일 기준 배율**(=per-token × seq_len)과 **지시어/코드 비율**로 정규화하면:

| 모델 | 지시어 배율(×균일) | 지시어/코드 비율 | 판정 |
|---|---|---|---|
| qwen | **4.1×** | 1.88 | 지시어 잘 읽힘 |
| deepseek | 1.3× | 1.75 | 읽힘 |
| stability | 1.9× | 1.74 | 읽힘 |
| **granite** | **0.7×** (균일 이하) | **0.61** (코드보다↓) | **미주목** |

- **3/4(qwen·deepseek·stability)**: 지시어가 균일 이상 + 코드만큼/이상 → **분명히 읽힘, 미주목 아님.**
- **granite만 예외**: 지시어가 **균일보다도 낮고 코드보다도 낮음** → 여기선 진짜 미주목. (raw로 봤으면 놓쳤을, 정규화로 드러난 예외.)
- → **Spotlight 크기 전제("지침 어텐션 부족")는 3/4에서 실패**, granite에서만 전제가 성립할 여지. **다수에서 실패**가 핵심이나 **보편은 아님**(§4).
