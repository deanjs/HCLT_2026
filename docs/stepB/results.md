...
1. **위치의 의미.** L25 = 전체 36층의 **약 70%(후반부)**. 계획서 §2.5 — 초기=어휘, 중간=구문, **후반=출력 결정** — 에서 L25는 **출력 결정 단계**. 해석: **표기 형태 선택이 출력 직전에 굳는다**(step 7 로짓 렌즈로 실증 예정).
...
- **한계로 기록.** 밑줄이 신호인지는 경계 토큰 격리 또는 step 7 로짓 렌즈로 이관.
...
- **밑줄 자체가 신호인지**는 토큰화가 `_`를 명사에 붙여 격리 불가 → **step 7**(로짓 렌즈).
