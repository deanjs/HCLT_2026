...
### 4.3 전체 per-token 분포 — 20종씩, 두 그룹 모두

**`code_snake` — 토큰 텍스트별 av 분포 (전 140조건), 20 동사 · 20 명사 전부:**
...
- **20 동사 전부가 명사보다 위쪽**에 몰린다: 앞 동사 av 평균 **0.110** [0.078–0.152] vs 뒤 명사(밑줄) **0.075** [0.040–0.103]. 분포가 조금 겹치지만(최고 명사 `_index` 0.103 &gt; 최저 동사 `encode` 0.078) 전반적으로 **query는 이름의 앞 동사 토큰을 더 본다.**

**`code_camel` — 20 동사 · 20 대문자명사 전부 (av 분포):**

![camel 전체 per-token av](figs/token_av_camel_full.png)

camel은 반대다 — **뒤 대문자 명사(`Cursor`, `Record` …)가 앞 동사보다 av가 크다**(§4.2에서 봤듯 v가 커서). snake와 방향이 정반대라, "마커 토큰이 신호"라는 해석이 성립하지 않는다.
...
- **밑줄 자체가 신호인지**는 토큰화가 `_`를 명사에 붙여 격리 불가 → **step 8**(로짓 렌즈).
