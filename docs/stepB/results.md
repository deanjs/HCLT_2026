...
**중요:** `code_camel`은 **지침 안의 camelCase 단어가 아니라 선행 코드의 camel 이름**이다. 그 이름은 다시 **하위 토큰**으로 쪼개진다 — `formatMatrix → [' format', 'Matrix']`, `validate_stream → [' validate', '_stream']`. 어느 글자에 어텐션이 가는지는 §5 per-token 표에서 토큰 단위로 볼 수 있다.
...
**토큰 텍스트별 av 집계 (전 140조건):**

![per-token av](figs/token_av.png)

| 상위 (av 큼) | av | 하위 (av 작음) | av |
|---|---|---|---|
| `' validate'` (동사) | 0.152 | `'_config'` | 0.066 |
| `' expand'` (동사) | 0.145 | `'_record'` | 0.062 |
| `' collect'` (동사) | 0.131 | `'_entry'` | 0.057 |
| `' resolve'` (동사) | 0.128 | `'_header'` | 0.040 |

- **첫 단어(동사) av 평균 0.110 vs 둘째 단어(밑줄+명사) 0.075.** 상위는 전부 동사, 하위는 전부 `_`+명사. 즉 query는 이름의 **앞 동사 토큰**을 더 본다(위치·품사 효과).
