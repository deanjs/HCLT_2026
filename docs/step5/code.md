...
> 아래는 **코드**다. 이 변수들에 **실제로 무슨 글자가 들어 있는지**는
> [`방법론.md` §2-1](방법론.md)에 프롬프트 원문으로 찍혀 있고, 그걸 어떤 순서로 쓰는지는
> §2-2에 절차로 적혀 있다. **"대체 뭘 어떻게 잰 건가"가 안 잡히면 거기부터 본다.**
...
| `self` | **같은 지침의 같은 지시어**(평균 낸 뒤 덮음 — 항등 아님) | 전이 **≈ 0** | 새 정보 없이 남는 **평균 풀링 + 덮어쓰기 교란** |
| `unrelated_word` | 같은 지침의 `"project"` | 전이 **≈ 0** | 표기와 무관한 내용을 덮었을 때의 기준선 |

**`self` 통제가 이 연구에서 가장 예리한 도구다.** 다만 **항등 개입이 아니다** —
`token_unit="mean"`이므로 같은 단어의 조각들을 **평균 낸 뒤** 모든 자리에 덮는다:

```
(k₁, k₂)  →  ((k₁+k₂)/2, (k₁+k₂)/2)      ← 단어 내부 구조가 뭉개진다
```

그래서 `self`가 잡아내는 것은 "덮어쓰는 행위"만이 아니라 **평균 풀링 + 덮어쓰기 교란 전체**다.
처치도 같은 교란을 겪으므로 빼는 것은 옳지만, **해석할 때 이름을 정확히 불러야 한다.**

실제 결과: **Key 처치(0.106)와 Key 자기통제(0.107)가 소수점 셋째 자리까지 같았다.**

> **말할 수 있는 것:** 이 평균 덮어쓰기 규격에서 **반대 표기 공여가 자기 풀링 교란을 넘는
> 추가 효과를 거의 만들지 못했다.**
> **말할 수 없는 것:** "어텐션 경로가 지침 정보를 나르지 않는다." 그건 native Key 경로에 대한
> 주장인데, 우리는 post-RoPE 캐시를 평균으로 덮은 조건만 관측했다.

`"project"`가 중립어로 쓰이는 근거(**완전한 음성 통제는 아니다** — 표기어와 의미·토큰 수·문장 내 위치가 다르다):
...
⚠️ **통제 2,016개 중 분석에 쓰는 것은 672개뿐이다.** 같은 672조건(4모델 × 42묶음 ×
방향 2 × 공여 2)을 **세 가지 방식으로 돌렸는데**, 앞의 두 가지는 K+V만 쟀다:

| 실행 방식 | n | 쓰나 |
|---|---|---|
| 단일 층, **K+V만** | 672 | ✗ 방식 분해 없음 |
| 봉우리±3층(7층), **K+V만** | 672 | ✗ 같은 이유 |
| 단일 층, **key·value·key_value 전부** | **672** | ✓ **이것만** |

앞의 두 개는 `Intervention.kinds`를 추가하기 전의 실행분이다(당시 노트북이 `KEY_VALUE`만
지정했다). `scripts/step5_net_effect.py`는 `mode == "intervene_sweep"`로 세 번째만 고른다.
결과는 불변이므로(§6) 지우지 않고 남긴다.
...
`n_substituted_tokens`가 보통 2다(step3은 20~30). 두 스텝의 절대값을
"지침이 코드보다 약하다"로 읽으면 안 된다.

> 다만 **"자리당 효과가 더 크다"고도 말할 수 없다.** KV 치환의 효과는 비선형이고
> 토큰별로 더해지지 않는다. 자리 수는 조건 차이로만 기록한다.
...
| 결론 | "지침도 Value로 흐른다" | **이 규격에서 Key 공여는 자기 풀링 교란을 넘지 못한다** | 통제를 뺐더니 Key 순효과가 ≈0. 단 native Key 경로에 대한 주장은 아니다 |
