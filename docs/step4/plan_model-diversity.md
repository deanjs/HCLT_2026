...
**한 줄:** step4(Qwen)에서 본 "지침 레버 = Value, L27"을 **deepseek/granite/stable**로 확장해,
...
2. **`token_unit='last'`.** 마지막 서브토큰만 1:1 정렬 → **항상 nsub=1, 스킵 없음**(모든 모델 비교
   가능). `'all'`은 whole-word라 크게 잡히지만 토큰 수 불일치 시 스킵 → 크로스모델엔 부적합.
   대가: last는 크기가 작게 잡히나(Qwen 0.11 vs whole-word 0.94) **Value vs Key 방향은 유지.**
   (step2 코드 실험이 쓴 방식과 동일. 부분-치환 모드는 도입하지 않음 — 합의.)
...
| 축 | 값 |
|---|---|
| 모델 | Qwen / deepseek / granite / stable |
| 방향 | camel→snake, snake→camel |
| 선행 | 균형 6/6, 전부위반 0/12 |
| 블록 | 0–41 (504 이름 전부) |
| 정렬 | token_unit='last' |
| 개입 | KV group 치환, 전 층 스윕, 지침 지시어 타깃 |

= 4 × 2 × 2 × 42 = **672 스윕.** 재개 지원(슬러그에 모델·블록·방향·선행·tok 포함).
...
| **gap≈0** | 지침 레버 없음 → 전이율 무의미, 별도 분류(granite 후보) |
