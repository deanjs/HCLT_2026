...
> ⚠️ **위 표는 봉우리 층 하나만 본 것이라 그 자체로 편향돼 있다** — 어느 층을 고르냐로
> 답이 뒤바뀐다. 그래서 **전 층을 센다**:
>
> | 모델 | 층수 | 방향 사이에 **부호가 뒤집히는** 층 | 유지되는 층 | 두 방향 평균이 0을 무는 층 |
> |---|---|---|---|---|
> | Qwen | 36 | **16** | 20 | 1 |
> | DeepSeek | 32 | **17** | 15 | 1 |
> | Llama | 28 | **23** | 5 | 0 |
> | StableCode | 32 | **23** | 9 | 0 |
>
> 네 모델 모두 층의 **절반 안팎에서 부호가 뒤집힌다.** 봉우리를 어디로 잡느냐에 따라
> "camel을 더 본다"도 "snake를 더 본다"도 만들 수 있다는 뜻이다. **못 쓴다.**

**재현:** `python scripts/step4_direction_control.py`
(①·② 표와 위 전 층 카운트를 함께 찍는다. 결과는 `results/step4_instr-observe/` 336개에서만 온다.)

**step2와 무엇이 달랐나**

| | step2 (코드 관측) | step4 (지침 관측) |
|---|---|---|
| 뒤집는 축 | 표기 배치 — **시드로 고정돼 있었다** | 지침 방향 — **처음부터 양쪽 다 돌렸다** |
| 통제 장치 | **없었다** → 시드 67을 새로 돌려야 했다 | **이미 데이터에 있었다** (추가 실행 0) |
| 주 지표 | 부호가 뒤집혀 **철회** | 두 방향 모두 유지 — **무사** |

step4가 양방향을 돌린 것은 원래 "지침이 camel일 때와 snake일 때 다른가"를 보려던 것인데,
결과적으로 **자리 통제 노릇까지 했다.** step2에는 그 장치가 없었다.
