...
**통제도 이제 전 층에서 돈다**(`results/step5_control_sweep_<모델>/`, 모델당 168조건).
그래서 **층별 순효과 곡선(`net_<모델>`)을 만들 수 있다** — step3에는 있는데 step5에는 없던
비대칭이 없어졌다.

    처치(opposite)        전 층      28~36개 층
    통제(self·무관어)      전 층      28~36개 층      ← 층마다 뺄 수 있다

옛 실행분(`results/step5_instr-cause-control/`)은 통제가 **층 하나뿐**이라 이 코드가
`full=False` 경로로 떨어져 통제를 **수평선**으로 그린다. 그 폴더만 있는 상태로 돌리면
`net_*`이 안 만들어지는 것이 정상이다.
