...
class spotlight_attention:
    ...
    **전 층·전 헤드에 같은 ψ를 거는 것은 원문의 설계다**(우리가 임의로 정한 것이 아니다).
    원문 근거: PASTA류의 오프라인 프로파일링(헤드를 하나씩 조정해 고르는 과정)을 생략하려는
    것이고, 층·헤드를 골라 거는 전략은 실험에서 변동성이 크고 기준 모델보다 나쁠 때도 있었다고
    보고한다. 지침 정보가 모델 전체에 분산돼 인코딩된다는 해석이다.

    헤드마다 **독립으로** ψ를 맞춘다는 점에 주의한다 — `before`가 [배치, 헤드, 쿼리위치, 1]
    이므로 배율이 헤드·쿼리위치마다 따로 계산된다. 헤드 전체를 합쳐 ψ 하나로 맞추는 것과 다르다.

    ⚠️ **쿼리 위치 적용 범위는 우리가 정했다** — 전 쿼리 위치에 건다(이름 쓰는 마지막 행만이
    아니다). 이 자유도는 원문에서 확인하지 못했다. Spotlight에 가장 넓게 준 설정이다.
    """
